...
void create_cmd(PVM vm, const char* name, char type, CMD_CB fnc, char precedence, const char* desc)
{
	register_command(vm, create_command(name, type, fnc, precedence, desc));
}
void register_commmands(PVM vm)
{
	if (find_command(vm, "+", 'b') != 0)
		return;
	create_cmd(vm, "+", 'b', CMD_PLUS, 8, "<SCALAR> + <SCALAR> | <STRING> + <STRING> | <ARRAY> + <ANY>");
	create_cmd(vm, "-", 'b', CMD_MINUS, 8, "<SCALAR> - <SCALAR> ");
	create_cmd(vm, "*", 'b', CMD_MULTIPLY, 9, "<SCALAR> * <SCALAR>");
	create_cmd(vm, "/", 'b', CMD_DIVIDE, 9, "<SCALAR> / <SCALAR>");
	create_cmd(vm, ">", 'b', CMD_LARGETTHEN, 7, "<SCALAR> > <SCALAR>");
	create_cmd(vm, "<", 'b', CMD_LESSTHEN, 7, "<SCALAR> < <SCALAR>");
	create_cmd(vm, ">=", 'b', CMD_LARGETTHENOREQUAL, 7, "<SCALAR> >= <SCALAR>");
	create_cmd(vm, "<=", 'b', CMD_LESSTHENOREQUAL, 7, "<SCALAR> <= <SCALAR>");
	create_cmd(vm, "==", 'b', CMD_EQUAL, 7, "<SCALAR> == <SCALAR> | <OBJECT> == <OBJECT> | <STRING> == <STRING>");
	create_cmd(vm, "!=", 'b', CMD_NOTEQUAL, 7, "<SCALAR> != <SCALAR> | <OBJECT> != <OBJECT> | <STRING> != <STRING>");
	create_cmd(vm, "||", 'b', CMD_OROR, 5, "<BOOL> || <BOOL> | <BOOL> || <CODE>");
	create_cmd(vm, "&&", 'b', CMD_ANDAND, 6, "<BOOL> && <BOOL> | <BOOL> && <CODE>");
	create_cmd(vm, "or", 'b', CMD_OROR, 5, "<BOOL> or <BOOL> | <BOOL> && <CODE>");
	create_cmd(vm, "and", 'b', CMD_ANDAND, 6, "<BOOL> and <BOOL>");
	create_cmd(vm, "select", 'b', CMD_SELECT, 10, "<ARRAY> select <SCALAR>");
	create_cmd(vm, "then", 'b', CMD_THEN, 5, "<IF> then <ARRAY>");
	create_cmd(vm, "else", 'b', CMD_ELSE, 6, "<CODE> else <CODE>");
	create_cmd(vm, "do", 'b', CMD_DO, 0, "<WHILE> do <CODE> | <FOR> do <CODE>");
	create_cmd(vm, "from", 'b', CMD_FROM, 0, "<FOR> from <SCALAR>");
	create_cmd(vm, "to", 'b', CMD_TO, 0, "<FOR> to <SCALAR>");
	create_cmd(vm, "step", 'b', CMD_STEP, 0, "<FOR> step <SCALAR>");
	create_cmd(vm, "count", 'b', CMD_COUNT, 0, "<CODE> count <ARRAY> | <COUNT> count <BOOL>");
	create_cmd(vm, "call", 'b', CMD_CALL, 0, "<ANY> call <CODE>");
	create_cmd(vm, "atan2", 'b', CMD_ATAN2, 0, "<SCALAR> atan2 <SCALAR>");
	create_cmd(vm, "min", 'b', CMD_MIN, 0, "<SCALAR> min <SCALAR>");
	create_cmd(vm, "max", 'b', CMD_MAX, 0, "<SCALAR> max <SCALAR>");
	create_cmd(vm, "mod", 'b', CMD_MOD, 0, "<SCALAR> mod <SCALAR>");
	create_cmd(vm, "%", 'b', CMD_MOD, 0, "<SCALAR> % <SCALAR>");
	create_cmd(vm, "^", 'b', CMD_POWEROF, 0, "<SCALAR> ^ <SCALAR>");
	create_cmd(vm, "pushBack", 'b', CMD_PUSHBACK, 0, "<ARRAY> pushBack <ANY>");
	create_cmd(vm, "set", 'b', CMD_SET, 0, "<ARRAY> set <ANY>");
	create_cmd(vm, "isEqualTo", 'b', CMD_ISEQUALTO, 0, "<ANY> isEqualTo <ANY>");
	create_cmd(vm, "createVehicle", 'b', CMD_CREATEVEHICLE, 0, "<STRING> createVehicle <ARRAY>");
	create_cmd(vm, "getVariable", 'b', CMD_GETVARIABLE, 0, "<NAMESPACE> getVariable <STRING> | <NAMESPACE> getVariable <ARRAY> | <OBJECT> getVariable <STRING> | <OBJECT> getVariable <ARRAY>");
	create_cmd(vm, "setVariable", 'b', CMD_SETVARIABLE, 0, "<NAMESPACE> setVariable <ARRAY> | <OBJECT> setVariable <ARRAY>");
	create_cmd(vm, "setPos", 'b', CMD_SETPOS, 0, " <OBJECT> setPos <ARRAY>");
	...
	create_cmd(vm, "if", 'u', CMD_IF, 0, "if <BOOL>");
	create_cmd(vm, "str", 'u', CMD_STR, 0, "str <ANY>");
	create_cmd(vm, "while", 'u', CMD_WHILE, 0, "while <CODE>");
	create_cmd(vm, "typeName", 'u', CMD_TYPENAME, 0, "typeName <ANY>");
	create_cmd(vm, "for", 'u', CMD_FOR, 0, "for <STRING>");
	create_cmd(vm, "+", 'u', CMD_PLUS_UNARY, 0, "+ <ARRAY> | + <SCALAR> | + <NaN>");
	create_cmd(vm, "-", 'u', CMD_MINUS_UNARY, 0, "- <SCALAR>");
	create_cmd(vm, "count", 'u', CMD_COUNT_UNARY, 0, "count <STRING> | count <ARRAY>");
	create_cmd(vm, "format", 'u', CMD_FORMAT, 0, "format <ARRAY>");
	create_cmd(vm, "call", 'u', CMD_CALL_UNARY, 0, "call <CODE>");
	create_cmd(vm, "abs", 'u', CMD_ABS, 0, "abs <SCALAR>");
	create_cmd(vm, "deg", 'u', CMD_DEG, 0, "deg <SCALAR>");
	create_cmd(vm, "log", 'u', CMD_LOG, 0, "log <SCALAR>");
	create_cmd(vm, "sin", 'u', CMD_SIN, 0, "sin <SCALAR>");
	create_cmd(vm, "cos", 'u', CMD_COS, 0, "cos <SCALAR>");
	create_cmd(vm, "tan", 'u', CMD_TAN, 0, "tan <SCALAR>");
	create_cmd(vm, "exp", 'u', CMD_EXP, 0, "exp <SCALAR>");
	create_cmd(vm, "asin", 'u', CMD_ASIN, 0, "asin <SCALAR>");
	create_cmd(vm, "acos", 'u', CMD_ACOS, 0, "acos <SCALAR>");
	create_cmd(vm, "atan", 'u', CMD_ATAN, 0, "atan <SCALAR>");
	create_cmd(vm, "atg", 'u', CMD_ATAN, 0, "atg <SCALAR>");
	create_cmd(vm, "rad", 'u', CMD_RAD, 0, "rad <SCALAR>");
	create_cmd(vm, "sqrt", 'u', CMD_SQRT, 0, "sqrt <SCALAR>");
	create_cmd(vm, "ceil", 'u', CMD_CEIL, 0, "ceil <SCALAR>");
	create_cmd(vm, "random", 'u', CMD_RANDOM, 0, "random <SCALAR>");
	create_cmd(vm, "floor", 'u', CMD_FLOOR, 0, "floor <SCALAR>");
	create_cmd(vm, "ln", 'u', CMD_LN, 0, "ln <SCALAR>");
	create_cmd(vm, "round", 'u', CMD_ROUND, 0, "round <SCALAR>");
	create_cmd(vm, "!", 'u', CMD_NOT, 0, "! <BOOL>");
	create_cmd(vm, "comment", 'u', CMD_COMMENT, 0, "comment <BOOL>");
	create_cmd(vm, "typeOf", 'u', CMD_TYPEOF, 0, "typeOf <OBJECT>");
	create_cmd(vm, "getPos", 'u', CMD_GETPOS, 0, "getPos <OBJECT>");
	create_cmd(vm, "position", 'u', CMD_GETPOS, 0, "position <OBJECT>");
	create_cmd(vm, "velocity", 'u', CMD_VELOCITY, 0, "velocity <OBJECT>");
	create_cmd(vm, "switch", 'u', CMD_SWITCH, 0, "switch <ANY>");
	create_cmd(vm, "case", 'u', CMD_CASE, 0, "case <ANY>");
	create_cmd(vm, "default", 'u', CMD_DEFAULT, 0, "default <CODE>");
