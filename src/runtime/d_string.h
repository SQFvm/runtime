...
namespace sqf {
    namespace runtime {
        struct t_string : public type::extend<t_string> {
            t_string() : extend() {}
    ...
    namespace types {
        ...
        public:
            ...
            std::string to_string_sqf() const override {
                size_t count = /* count quotes */
                        std::count(m_value.begin(), m_value.end(), '"') + 2 /* outter quotes */;
        ...
        template<>
        inline std::shared_ptr<sqf::runtime::data>
        to_data<const char *>(const char *str) { return std::make_shared<d_string>(std::string(str)); }

        template<>
        inline std::shared_ptr<sqf::runtime::data> to_data<char>(char value) {
            return std::make_shared<d_string>(std::string(&value, &value + 1));
        }

        template<>
        inline std::shared_ptr<sqf::runtime::data>
        to_data<std::string>(std::string str) { return std::make_shared<d_string>(str); }
        ...
        template<>
        inline std::shared_ptr<sqf::runtime::data>
        to_data<std::string_view>(std::string_view str) { return std::make_shared<d_string>(str); }
    }
}