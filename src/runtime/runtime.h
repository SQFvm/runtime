...
#pragma region Runtime Diagnostics
    ...
    public:
        ...
        void breakpoint_hit(sqf::runtime::diagnostics::breakpoint breakpoint) {
            m_last_breakpoint_hit = breakpoint;
            m_is_halt_requested = true;
        }
...
#pragma region Operators
    ...
    public:
        ...
        bool sqfop_exists(const sqf::runtime::sqfop_binary::key key) const {
            return m_operators_binary.find(key) != m_operators_binary.end();
        }

        sqf::runtime::sqfop_binary::cref
        sqfop_at(const sqf::runtime::sqfop_binary::key key) const { return m_operators_binary.at(key); }

        const std::vector<sqf::runtime::sqfop_binary::cwref> &
        sqfop_binary_by_name(const std::string key) const { return m_operators_by_name_binary.at(key); }
        ...
        bool sqfop_exists(const sqf::runtime::sqfop_unary::key key) const {
            return m_operators_unary.find(key) != m_operators_unary.end();
        }

        sqf::runtime::sqfop_unary::cref
        sqfop_at(const sqf::runtime::sqfop_unary::key key) const { return m_operators_unary.at(key); }

        const std::vector<sqf::runtime::sqfop_unary::cwref> &
        sqfop_unary_by_name(const std::string key) const { return m_operators_by_name_unary.at(key); }
        ...
        bool sqfop_exists(const sqf::runtime::sqfop_nular::key key) const {
            return m_operators_nular.find(key) != m_operators_nular.end();
        }

        sqf::runtime::sqfop_nular::cref
        sqfop_at(const sqf::runtime::sqfop_nular::key key) const { return m_operators_nular.at(key); }
...
#pragma region Namespaces
    ...
    public:
        ...
        std::shared_ptr<sqf::runtime::value_scope> default_value_scope() {
            return get_value_scope(m_default_scope_key);
        }
    ...
    public:
        ...
        sqf::runtime::context &context_active() {
            if (m_context_active) { return *m_context_active; }
                ...
                m_context_active = context_create().lock();
                return *m_context_active;
            }
            else {
                m_context_active = m_contexts.front();
                return *m_context_active;
            }
        };
        ...
        std::weak_ptr<context> context_create() {
            auto ptr = std::make_shared<context>();
            m_contexts.push_back(ptr);
            return ptr;
        }

        bool context_remove(std::shared_ptr<sqf::runtime::context> cntxt) {
            auto iter = std::find(m_contexts.begin(), m_contexts.end(), cntxt);
...
#pragma endregion
    ...
    public:
        ...
        void parser_preprocessor(
                std::unique_ptr<sqf::runtime::parser::preprocessor> ptr) { m_parser_preprocessor = std::move(ptr); }
