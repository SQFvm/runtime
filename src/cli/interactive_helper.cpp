...
void interactive_helper::virtualmachine_thread() {
    auto previous_state = sqf::runtime::runtime::state::empty;
    while (!m_thread_die) {
        ...
        switch (tmp) {
            case sqf::runtime::runtime::action::invalid:
                ...
                break;
            default:
                m_runtime.execute(tmp);
                break;
        }

        auto current_state = m_runtime.runtime_state();
...
void interactive_helper::init() {
    ...
    register_command(std::array{"rbp"s, "remove-breapoint"s},
                     ...
                     [](interactive_helper &interactive, std::string_view arg) -> void {
                         ...
                         line = std::stol(arg.data());
                         file = arg.substr(split);
                         auto res = std::find_if(interactive.runtime().breakpoints_begin(),
                                                 interactive.runtime().breakpoints_end(),
    ...
    register_command(std::array{"rbpa"s, "clear-breapoints"s},
                     ...
                     [](interactive_helper &interactive, std::string_view arg) -> void {
                         interactive.runtime()
                                 .erase(interactive.runtime().breakpoints_begin(),
                                        interactive.runtime().breakpoints_end());
                     });
                     ...
                     [](interactive_helper &interactive, std::string_view arg) -> void {
                         switch (interactive.runtime().runtime_state()) {
                             ...
                             default: {
                                 ...
                                 if (!selected_context) {
                                     auto active_context = interactive.runtime().context_active_as_shared();
                                     if (!active_context) {
                                         std::cerr << "Neither selected context nor active context available."
                                                   << std::endl;
                                         return;
                                     }
                                     selected_context = active_context;
                                     interactive.context_selected(active_context);
                                     std::cout << "Selected context no longer available. Changing to Currently Active."
                                               << std::endl;
                                 }

                                 // Dump frames to console
                                 if (selected_context->frames_rbegin() == selected_context->frames_rend())
                                     std::cout << "No frames available." << std::endl;
                                 int i = 0;
                                 for (auto it = selected_context->frames_rbegin();
                                      it != selected_context->frames_rend(); ++it) {
                     ...
                     [](interactive_helper &interactive, std::string_view arg) -> void {
                         switch (interactive.runtime().runtime_state()) {
                             ...
                             default: {
                                 ...
                                 if (selected_context->frames_rbegin() == selected_context->frames_rend())
                                     std::cout << "No frames available." << std::endl;
                                 for (auto it_frame = selected_context->frames_rbegin();
                                      it_frame != selected_context->frames_rend(); ++it_frame) {
                                     for (auto it_var = it_frame->begin(); it_var != it_frame->end(); ++it_var) {
                                         std::cout
                                         << "[" << std::setw(3) << space_index++ << "]"
                                         << "[F" << std::setw(3) << (selected_context->frames_rend() - it_frame) << "]"
                                         << " '" << it_var->first
                                                   << "' := " << it_var->second.to_string_sqf() << std::endl;
                                     }
                                 }
                             }
                                 break;
    ...
    register_command(std::array{"ls"s, "scripts"s, "list-scripts"s},
                     ...
                     [](interactive_helper &interactive, std::string_view arg) -> void {
                         if (interactive.runtime().context_begin() == interactive.runtime().context_end())
                             std::cout << "No scripts available." << std::endl;
                         size_t i = 0;
                         auto selected = interactive.context_selected();
                         for (auto it = interactive.runtime().context_begin();
                              it != interactive.runtime().context_end(); ++it) {
                             if ((*it)->name().empty()) {
                             ...
                             default: {
                                 ...
                                 for (auto it = interactive.runtime().context_begin();
                                     ...
                                     if (i == index++) {
                                         ...
                                         if ((*it)->name().empty()) {
                                             std::cout << "Selected context " << index << " '" << (*it)->name() << "'"
                                                       << std::endl;
                                         } else {
