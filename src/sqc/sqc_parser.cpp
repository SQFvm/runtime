...
void sqf::sqc::parser::to_assembly(::sqf::runtime::runtime& runtime, std::vector<::sqf::runtime::instruction::sptr>& set, std::vector<std::string>& locals, ::sqf::sqc::bison::astnode& node)
{
	...
	case ::sqf::sqc::bison::astkind::RETURN: {
		if (node.children.empty())
		{
			...
			set.push_back(std::make_shared<opcodes::call_unary>("breakout"s));
		}
		else
		{
			to_assembly(runtime, set, locals, node.children[0]);
			set.push_back(std::make_shared<opcodes::push>(__scopename_function));
			set.push_back(std::make_shared<opcodes::call_binary>("breakout"s, (short)4));
		}
	} break;
	case ::sqf::sqc::bison::astkind::THROW: {
		to_assembly(runtime, set, locals, node.children[0]);
		set.push_back(std::make_shared<opcodes::call_unary>("throw"s));
	} break;
	case ::sqf::sqc::bison::astkind::ASSIGNMENT: {
		...
		{
			set.push_back(std::make_shared<opcodes::assign_to>("_" + var));
		}
		else
		{
			set.push_back(std::make_shared<opcodes::assign_to>(var));
		}
	} break;
	case ::sqf::sqc::bison::astkind::DECLARATION: {
		...
		set.push_back(std::make_shared<opcodes::assign_to_local>("_"s + var));
	} break;
	case ::sqf::sqc::bison::astkind::FORWARD_DECLARATION: {
		set.push_back(std::make_shared<opcodes::push>(node.children[0].token.contents));
		set.push_back(std::make_shared<opcodes::call_unary>("private"s));
	} break;
	case ::sqf::sqc::bison::astkind::FUNCTION_DECLARATION: {
		...
		set.push_back(std::make_shared<opcodes::assign_to>(std::string(node.children[0].token.contents)));
	} break;
	...
	case ::sqf::sqc::bison::astkind::ARGLIST: {
		...
		set.push_back(std::make_shared<opcodes::call_unary>("params"s));
	} break;
	...
	case ::sqf::sqc::bison::astkind::IF: {
		// Emit condition
		to_assembly(runtime, set, locals, node.children[0]);
		set.push_back(std::make_shared<opcodes::call_unary>("if"s));

		std::vector<::sqf::runtime::instruction::sptr> local_set;
		to_assembly(runtime, local_set, locals, node.children[1]);
		set.push_back(std::make_shared<opcodes::push>(runtime::instruction_set{ local_set }));

		set.push_back(std::make_shared<opcodes::call_binary>("then"s, (short)4));
	} break;
	...
	case ::sqf::sqc::bison::astkind::IFELSE: {
		...
		// Emit on-true
		std::vector<::sqf::runtime::instruction::sptr> local_set1;
		to_assembly(runtime, local_set1, locals, node.children[1]);
		set.push_back(std::make_shared<opcodes::push>(runtime::instruction_set{ local_set1 }));

		// Emit on-false
		std::vector<::sqf::runtime::instruction::sptr> local_set2;
		to_assembly(runtime, local_set2, locals, node.children[2]);
		set.push_back(std::make_shared<opcodes::push>(runtime::instruction_set{ local_set2 }));
		set.push_back(std::make_shared<opcodes::call_binary>("else"s, (short)5));
	...
	case ::sqf::sqc::bison::astkind::FOR: {
		...
		set.push_back(std::make_shared<opcodes::push>(for_lvar));
		set.push_back(std::make_shared<opcodes::call_unary>("for"s));

		// Emit "from"
		to_assembly(runtime, set, locals, node.children[1]);
		set.push_back(std::make_shared<opcodes::call_binary>("from"s, (short)4));

		// Emit "to"
		to_assembly(runtime, set, locals, node.children[2]);
		set.push_back(std::make_shared<opcodes::call_binary>("to"s, (short)4));
		...
		// Emit "do"
		set.push_back(std::make_shared<opcodes::call_binary>("do"s, (short)4));
	} break;
	case ::sqf::sqc::bison::astkind::FORSTEP: {
		...
		set.push_back(std::make_shared<opcodes::call_binary>("step"s, (short)4));
	...
	case ::sqf::sqc::bison::astkind::FOREACH: {
		...
		// Emit Codeblock
		{
			...
			set.push_back(std::make_shared<opcodes::get_variable>("_x"s));
			std::string var(node.children[0].token.contents);
			std::string lvar = "_"s + var;
			set.push_back(std::make_shared<opcodes::assign_to_local>(lvar));
		...
		// Emit "forEach"
		set.push_back(std::make_shared<opcodes::call_binary>("foreach"s, (short)4));
	} break;
	case ::sqf::sqc::bison::astkind::WHILE: {
		...
		set.push_back(std::make_shared<opcodes::call_binary>("while"s, (short)4));
	...
	case ::sqf::sqc::bison::astkind::DOWHILE: {
		...
		// Emit call before while for the "do once"
		{
			...
			set.push_back(std::make_shared<opcodes::call_unary>("call"s));
		}
	...
	case ::sqf::sqc::bison::astkind::TRYCATCH: {
		// Emit call before while for the "do once"
		{
			...
			// Use unary call to call created scope
			set.push_back(std::make_shared<opcodes::call_unary>("try"s));
		}
		...
		set.push_back(std::make_shared<opcodes::call_binary>("catch"s, (short)4));
	} break;
	case ::sqf::sqc::bison::astkind::SWITCH: {
		// Emit switch
		to_assembly(runtime, set, locals, node.children[0]);
		set.push_back(std::make_shared<opcodes::call_unary>("switch"s));
	...
	case ::sqf::sqc::bison::astkind::CASE: {
		// Emit "case"
		to_assembly(runtime, set, locals, node.children[0]);
		set.push_back(std::make_shared<opcodes::call_unary>("case"s));
		...
		if (node.children.size() == 2) {
			...
			// Emit "colon"
			set.push_back(std::make_shared<opcodes::call_binary>(":"s, (short)4));
		}
	} break;
	case ::sqf::sqc::bison::astkind::CASE_DEFAULT: {
		...
		// Emit "default"
		set.push_back(std::make_shared<opcodes::call_unary>("default"s));
	} break;
	case ::sqf::sqc::bison::astkind::OP_OR: {
		...
		// Emit "or"
		set.push_back(std::make_shared<opcodes::call_binary>("||"s, (short)1));
	} break;
	case ::sqf::sqc::bison::astkind::OP_AND: {
		...
		// Emit "and"
		set.push_back(std::make_shared<opcodes::call_binary>("&&"s, (short)2));
	} break;
	case ::sqf::sqc::bison::astkind::OP_EQUALEXACT: {
		...
		// Emit "==="
		set.push_back(std::make_shared<opcodes::call_binary>("isequalto"s, (short)4));
	} break;
	case ::sqf::sqc::bison::astkind::OP_EQUAL: {
		...
		// Emit "=="
		set.push_back(std::make_shared<opcodes::call_binary>("=="s, (short)3));
	} break;
	case ::sqf::sqc::bison::astkind::OP_NOTEQUALEXACT: {
		...
		// Emit "!=="
		set.push_back(std::make_shared<opcodes::call_binary>("isequalto"s, (short)4));
		set.push_back(std::make_shared<opcodes::call_unary>("!"s));
	} break;
	...
	case ::sqf::sqc::bison::astkind::OP_PLUS: {
		...
		// Emit "+"
		set.push_back(std::make_shared<opcodes::call_binary>("+"s, (short)6));
	} break;
	...
	case ::sqf::sqc::bison::astkind::OP_MULTIPLY: {
		...
		// Emit "*"
		set.push_back(std::make_shared<opcodes::call_binary>("*"s, (short)7));
	} break;
	...
	case ::sqf::sqc::bison::astkind::OP_BINARY: {
		...
		if (runtime.sqfop_exists_binary(opname))
		{
			...
			if (node.children[2].children.size() > 1 || matchingOps.front().get().right_type() == runtime::t_array())
			{
				// Emit Right-Argument
				to_assembly(runtime, set, locals, node.children[2]);
				set.push_back(std::make_shared<opcodes::make_array>(std::max(node.children[2].children.size(), (size_t)1)));
			}
			else
			{
				// Emit Right-Argument
			...
			set.push_back(std::make_shared<opcodes::call_binary>(std::string(matchingOps.front().get().name()), matchingOps.front().get().precedence()));
		}
		else
		{
	...
	case ::sqf::sqc::bison::astkind::OP_UNARY: {
		...
		if (runtime.sqfop_exists_unary(opname))
		{
			...
			// Emit binary operator
			set.push_back(std::make_shared<opcodes::call_unary>(std::string(matchingOps.front().get().name())));
		}
		else
		{
			// Emit Right-Argument
	...
	case ::sqf::sqc::bison::astkind::VAL_STRING: {
		set.push_back(std::make_shared<opcodes::push>(types::d_string::from_sqf(node.token.contents)));
	} break;
	...
	case ::sqf::sqc::bison::astkind::VAL_TRUE: {
		set.push_back(std::make_shared<::sqf::opcodes::push>(true));
	} break;
	case ::sqf::sqc::bison::astkind::VAL_FALSE: {
		set.push_back(std::make_shared<::sqf::opcodes::push>(false));
	} break;
