...
void ::sqf::parser::sqf::parser::to_assembly(
        std::string_view contents,
        const ::sqf::parser::sqf::bison::astnode &node,
        std::vector<::sqf::runtime::instruction::sptr> &set) {
    switch (node.kind) {
        ...
        case bison::astkind::EXP9: {
            ...
            auto inst = std::make_shared<::sqf::opcodes::call_binary>(s, (short) (
                    ((short) node.kind - (short) bison::astkind::EXP0) + 1));
            inst->diag_info({node.token.line, node.token.column, node.token.offset, {*node.token.path, {}},
                             create_code_segment(contents, node.token.offset, node.token.contents.length())});
            set.push_back(inst);
            break;
        }
        case bison::astkind::EXP_GROUP: {
            to_assembly(contents, node.children[0], set);
            break;
        }
        ...
        case bison::astkind::HEXNUMBER: {
            try {
                ...
                auto inst = std::make_shared<::sqf::opcodes::push>(
                        ::sqf::runtime::value(std::make_shared<::sqf::types::d_scalar>(std::nanf(""))));
                inst->diag_info({node.token.line, node.token.column, node.token.offset, {*node.token.path, {}},
                                 create_code_segment(contents, node.token.offset, node.token.contents.length())});
        ...
        case bison::astkind::NUMBER: {
            try {
                auto inst = std::make_shared<::sqf::opcodes::push>(::sqf::runtime::value(
                        std::make_shared<::sqf::types::d_scalar>(
                                (double) std::stod(std::string(node.token.contents)))));
                inst->diag_info({node.token.line, node.token.column, node.token.offset, {*node.token.path, {}},
                                 create_code_segment(contents, node.token.offset, node.token.contents.length())});
                set.push_back(inst);
            }
        ...
        case bison::astkind::STRING: {
            auto inst = std::make_shared<::sqf::opcodes::push>(::sqf::runtime::value(
                    std::make_shared<::sqf::types::d_string>(::sqf::types::d_string::from_sqf(node.token.contents))));
            inst->diag_info({node.token.line, node.token.column, node.token.offset, {*node.token.path, {}},
                             create_code_segment(contents, node.token.offset, node.token.contents.length())});
            set.push_back(inst);
            break;
        ...
        case bison::astkind::CODE: {
            ...
            for (size_t i = 0; i < node.children.size(); i++) {
                if (i != 0) {
                    ...
                    inst->diag_info({previous_node.token.line,
                                     previous_node.token.column + previous_node.token.contents.length(),
                                     previous_node.token.offset, {*previous_node.token.path, {}},
                                     create_code_segment(contents, previous_node.token.offset,
                                                         previous_node.token.contents.length())});
            ...
            auto inst = std::make_shared<::sqf::opcodes::push>(
                    ::sqf::runtime::value(std::make_shared<::sqf::types::d_code>(inst_set)));
            inst->diag_info({node.token.line, node.token.column, node.token.offset, {*node.token.path, {}},
                             create_code_segment(contents, node.token.offset, node.token.contents.length())});
            set.push_back(inst);
            break;
